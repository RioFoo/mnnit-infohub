  ...
  return (
    <div className="page-container">
      <motion.div
        ...
        className="mb-8"
      >
        <h1 className="text-2xl md:text-3xl page-header gradient-text">Campus</h1>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="relative mb-10 max-w-xl">
        ...
        <Input
          ...
          className="pl-10 h-11 rounded-xl bg-muted/20 border-border/20 focus:border-primary/30 transition-all text-sm"
        />
      </motion.div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {QUICK_LINKS.map((link, i) => (
            ...
            >
              ...
              <p className="text-[10px] text-muted-foreground mt-1 capitalize">{link.category}</p>
            </motion.a>
          ))}
        </div>
      </section>
      ...
      <section className="mb-12">
        ...
        <Tabs defaultValue={CONTACT_DIRECTORY[0].category}>
          <TabsList className="flex-wrap h-auto rounded-xl bg-muted/20 border border-border/15 p-1">
            {CONTACT_DIRECTORY.map(cat => (
              <TabsTrigger key={cat.category} value={cat.category} className="text-[11px] rounded-lg font-medium data-[state=active]:bg-primary/10 data-[state=active]:text-primary">{cat.category}</TabsTrigger>
            ))}
          </TabsList>
          {CONTACT_DIRECTORY.map(cat => (
          ...
          {NOTICE_CATS.map(c => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[10px] font-medium transition-all duration-200 ${
                noticeCat === c ? 'bg-primary text-primary-foreground' : 'bg-muted/30 text-muted-foreground hover:bg-muted/50 border border-border/15'
              }`}
            >{c}</button>
          ))}
        </div>
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="text-right shrink-0">
                  <Badge variant="secondary" className="text-[10px] rounded-md font-medium">{n.category}</Badge>
        ...
        <div className="flex gap-2 flex-wrap mb-4">
          {CLUB_CATS.map(c => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[10px] font-medium transition-all duration-200 ${
                clubCat === c ? 'bg-primary text-primary-foreground' : 'bg-muted/30 text-muted-foreground hover:bg-muted/50 border border-border/15'
              }`}
            >{c}</button>
          ))}
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {filteredClubs.map((club, i) => (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: i * 0.03 }}
              className="float-card p-5 text-center group"
            >
              <div className="w-10 h-10 rounded-xl bg-primary/6 flex items-center justify-center mx-auto mb-3 group-hover:bg-primary/10 transition-colors">
                <Users className="w-5 h-5 text-primary" />
              </div>
              <p className="text-sm font-medium group-hover:text-primary transition-colors">{club.name}</p>
              <Badge variant="secondary" className="text-[10px] mt-2 rounded-md font-medium">{club.category}</Badge>
            </motion.div>
          ))}
        </div>
      </section>
