...
const CHART_COLORS = ['hsl(217, 91%, 60%)', 'hsl(263, 70%, 58%)', 'hsl(280, 65%, 55%)', 'hsl(45, 95%, 55%)', 'hsl(155, 70%, 50%)', 'hsl(0, 72%, 55%)'];
...
const Grades = () => {
  ...
  return (
    <div className="page-container">
      ...
      >
        <h1 className="text-2xl md:text-3xl page-header gradient-text">Grades</h1>
        <div className="text-right">
          <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-medium">CPI</p>
          <p className="text-3xl font-bold gradient-text" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{cpi.toFixed(2)}</p>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xs font-medium text-muted-foreground">SPI Trend</span>
          </div>
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={spiData}>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border) / 0.2)" />
              <XAxis dataKey="sem" tick={{ fontSize: 10 }} stroke="hsl(var(--muted-foreground) / 0.2)" />
              <YAxis domain={[0, 10]} tick={{ fontSize: 10 }} stroke="hsl(var(--muted-foreground) / 0.2)" />
              <Tooltip contentStyle={{ background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: '12px', fontSize: '12px' }} />
              <Line type="monotone" dataKey="spi" stroke="hsl(var(--primary))" strokeWidth={2} dot={{ fill: 'hsl(var(--primary))', r: 4, strokeWidth: 0 }} activeDot={{ r: 6 }} />
            </LineChart>
          </ResponsiveContainer>
        </motion.div>
      ...
      {/* Target */}
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="float-card p-5 mb-6">
        <div className="flex items-center gap-3 flex-wrap">
          <div className="w-8 h-8 rounded-lg bg-primary/8 flex items-center justify-center">
            <Target className="w-4 h-4 text-primary" />
          </div>
          <Input type="number" step="0.1" min="0" max="10" placeholder="Target CPI" value={targetCPI} onChange={e => setTargetCPI(e.target.value)} className="w-32 rounded-xl bg-muted/20 border-border/20 font-mono text-sm" />
      ...
      {semesters.map((sem, si) => (
        <motion.div
          ...
          className="float-card p-5 mb-4"
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold flex items-center gap-2" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
              Semester {sem.id}
              <Badge className="rounded-md bg-primary/8 text-primary border-primary/10 text-[10px] font-medium">SPI: {calcSPI(sem.subjects).toFixed(2)}</Badge>
            </h3>
            <div className="flex gap-2">
              <Button variant="ghost" size="sm" onClick={() => addSubject(sem.id)} className="rounded-lg text-xs">
          ...
          <div className="space-y-2">
            {sem.subjects.map((sub, i) => (
              <div key={i} className="flex gap-2 items-center">
                <Input placeholder="Subject" value={sub.name} onChange={e => updateSubject(sem.id, i, 'name', e.target.value)} className="flex-1 rounded-xl bg-muted/15 border-border/15 text-sm" />
                <Input type="number" placeholder="Cr" value={sub.credits} onChange={e => updateSubject(sem.id, i, 'credits', parseInt(e.target.value) || 0)} className="w-16 rounded-xl bg-muted/15 border-border/15 text-sm font-mono" />
                <Select value={sub.grade} onValueChange={v => updateSubject(sem.id, i, 'grade', v)}>
                  <SelectTrigger className="w-20 rounded-xl bg-muted/15 border-border/15 text-sm font-mono"><SelectValue /></SelectTrigger>
      ...
      <Button variant="outline" onClick={addSemester} className="w-full rounded-xl border-border/20 text-sm btn-premium">
