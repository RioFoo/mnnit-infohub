...
const Profile = () => {
  ...
  const handleSave = async () => {
    if (!user) return;
    setSaving(true);
    const { error } = await supabase.from('profiles').update({ name, bio, role }).eq('id', user.id);
          ...
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <Input value={role} onChange={e => setRole(e.target.value)} placeholder="e.g., CSE Student, 2nd Year" />
              </div>
